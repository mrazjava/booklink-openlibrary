...
interface DepotSearch<D> {
    ...
    public ResponseEntity<List<D>> randomRecord(
            ...
            @ApiParam(value = "with SMALL image? (requires operator)") @RequestParam(value = "img-s", required = false) Boolean imgS,
            @ApiParam(value = "with MEDIUM image? (requires operator)") @RequestParam(value = "img-m", required = false) Boolean imgM,
            @ApiParam(value = "with LARGE image? (requires operator)") @RequestParam(value = "img-l", required = false) Boolean imgL,
    ...
    @ApiOperation(value = "Find records matching a group of IDs, optionally exluding image(s)")
    ...
    public ResponseEntity<List<D>> findAllById(
            @ApiParam(value = "comma separated list of IDs", required = true) @RequestParam("ids") String commaSeparatedIds,
            @ApiParam(value = "return small image? (default: false)") @RequestParam(value = "img-s", required = false) Boolean imgS,
            @ApiParam(value = "return medium image? (default: false)") @RequestParam(value = "img-m", required = false) Boolean imgM,
            @ApiParam(value = "return large image? (default: false)") @RequestParam(value = "img-l", required = false) Boolean imgL);
